...
function finalizeTestRecord(user, test) {
    return new Q(particle.get_particle_device_from_uuid(user, test._device))
        ...
        .then(function(cartridge) {
            ...
            return Test.findByIdAndUpdate(test._id, updateQuery, {
                new: true
            }).exec();
        })
        .then(function(updatedTest) {
            return [updatedTest, Device.findByIdAndUpdate(test._device, {
                claimed: false
            }).exec()];
        });
}
...
exports.cancel = function(req, res) {
    particle.get_particle_device_from_uuid(req.user, req.body.deviceID)
        ...
        .then(function(result) {
            ...
            return Cartridge.findByIdAndUpdate(req.body.cartridgeID, updateObj, {
                new: true
            }).exec();
        })
        .then(function(cartridge) {
            ...
            return [cartridge, Test.findByIdAndUpdate(req.body.testID, updateObj, {
                new: true
            }).exec()];
        })
        .spread(function(cartridge, test) {
            ...
            return [cartridge, test, Device.findByIdAndUpdate(req.body.deviceID, updateObj, {
                new: true
            }).exec()];
        })
...
exports.exportable = function(req, res) {
    Test.find({
        status: 'Complete'
    }).limit(20).sort('-created').populate([{
        path: 'user',
        select: 'displayName'
    }, {
        path: '_assay',
