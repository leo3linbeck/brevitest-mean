...
describe('Spark CRUD tests', function() {
	...
	it('should not be able to save Spark instance if no sparkID is provided', function(done) {
		// Invalidate name field
		spark.sparkID = '';
		...
		agent.post('/auth/signin')
			...
			.end(function(signinErr, signinRes) {
				...
				agent.post('/sparks')
					...
					.end(function(sparkSaveErr, sparkSaveRes) {
						// Set message assertion
						(sparkSaveRes.body.message).should.match('Please fill Spark ID');
	...
	it('should be able to refresh the Spark list if signed in', function(done) {
		agent.post('/auth/signin')
			.send(credentials)
			.expect(200)
			.end(function(signinErr, signinRes) {
