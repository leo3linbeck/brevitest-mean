...
describe('Provider CRUD tests', function() {
			...
			.end(function(signinErr, signinRes) {
				...
				agent.post('/providers')
					...
					.end(function(providerSaveErr, providerSaveRes) {
						...
						agent.put('/providers/' + providerSaveRes.body._id)
							.send(provider)
							.expect(200)
	...
	it('should be able to get a single Provider if not signed in', function(done) {
		...
		// Save the Provider
		providerObj.save(function() {
			request(app).get('/providers/' + providerObj._id)
				.end(function(req, res) {
					// Set assertion
			...
			.end(function(signinErr, signinRes) {
				...
				agent.post('/providers')
					...
					.end(function(providerSaveErr, providerSaveRes) {
						...
						agent.delete('/providers/' + providerSaveRes.body._id)
							.send(provider)
							.expect(200)
	...
	it('should not be able to delete Provider instance if not signed in', function(done) {
		...
		providerObj.save(function() {
			...
			request(app).delete('/providers/' + providerObj._id)
			.expect(401)
			.end(function(providerDeleteErr, providerDeleteRes) {
				// Set message assertion
				(providerDeleteRes.body.message).should.match('User is not logged in');
